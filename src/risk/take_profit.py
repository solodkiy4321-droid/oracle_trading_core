...
ВАЖНО: сейчас возвращается ОДИН уровень TP с RR = default_ratio.
Частичных закрытий по нескольким уровням нет — это осознанное упрощение.

Следствие: PositionManager формально поддерживает breakeven_after_tp
и trailing_after_tp, но реально сработает только breakeven_after_tp=1
(и trailing_after_tp=1, если задан). trailing_after_tp >= 2 не сработает,
потому что второго уровня TP в плане нет.

Если в будущем понадобятся несколько уровней — расширять здесь,
в TakeProfitCalculator.calculate (возвращать несколько level'ов).
"""
